...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    ...
    return node->distance(*end_node); //end_node is a pointer , so dereference before you pass it to the function.
...
/* bool RoutePlanner::Compare(RouteModel::Node *i ,RouteModel::Node *j){
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    struct {
        bool operator()(RouteModel::Node *i, RouteModel::Node* j) const
        {   
            return ((i->g_value+i->h_value)>(j->g_value+j->h_value));
        }   
    }compare_g_and_h;

    std::sort(open_list.begin(),open_list.end(),compare_g_and_h);
